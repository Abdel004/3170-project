...
public class AdministratorMenu {
	...
	public void CreateTables() {
		try {
			...
			String sql2 = "CREATE TABLE IF NOT EXISTS book_category(" + "bcid INT NOT NULL,"
					+ "bcname VARCHAR(30) NOT NULL," + "PRIMARY KEY(bcid)" + ")";
			String sql3 = "CREATE TABLE IF NOT EXISTS libuser(" + "libuid CHAR(10) NOT NULL,"
					+ "name VARCHAR(25) NOT NULL," + "age INT NOT NULL," + "address VARCHAR(100) NOT NULL,"
					+ "ucid INT NOT NULL," + "PRIMARY KEY(libuid),"
					+ "FOREIGN KEY(ucid) REFERENCES user_category(ucid) ON DELETE CASCADE ON UPDATE CASCADE" + ")";
			String sql4 = "CREATE TABLE IF NOT EXISTS book(" + "callnum CHAR(8) NOT NULL,"
					+ "title VARCHAR(30) NOT NULL," + "publish DATE NOT NULL, " + "rating REAL,"
					+ "tborrowed INT DEFAULT '0' NOT NULL," + "bcid INT NOT NULL," + "PRIMARY KEY(callnum),"
			...
			String sql6 = "CREATE TABLE IF NOT EXISTS borrow(" + "libuid CHAR(10) NOT NULL,"
					+ "callnum CHAR(8) NOT NULL," + "copynum INT NOT NUll," + "checkout DATE NOT NULL,"
					+ "returndate DATE," + "PRIMARY KEY(libuid, callnum, copynum, checkout),"
			...
			String sql7 = "CREATE TABLE IF NOT EXISTS authorship(" + "aname VARCHAR(25) NOT NULL,"
					+ "callnum CHAR(8) NOT NULL," + "PRIMARY KEY(aname, callnum),"
					+ "FOREIGN KEY(callnum) REFERENCES book(callnum) ON DELETE CASCADE ON UPDATE CASCADE" + ")";
			stm.addBatch(sql1);
			stm.addBatch(sql2);
			stm.addBatch(sql3);
	...
	public void DeleteTables() {
		try {
			...
			String sql2 = "SELECT table_name " + "FROM information_schema.tables " + "WHERE table_schema = 'db56'";
		...
		if (directoryPath.list() != null) {
			...
			while (sc.hasNextLine()) {
				try {
					...
					try {
						PreparedStatement pstmt = this.conn.prepareStatement(
								"INSERT INTO borrow(libuid,callnum,copynum,checkout,returndate) VALUES (?,?,?,?,?)");
						String date1 = dateString[2] + "/" + dateString[1] + "/" + dateString[0];
